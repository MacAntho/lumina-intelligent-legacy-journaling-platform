...
import type {
  User, Journal, Entry, LegacyContact, LegacyShare,
  ExportLog, LegacyAuditLog, AppNotification, SavedSearch,
  DailyContent, AiInsight, SecurityLog, UsageStats
} from "@shared/types";
...
export class EntryEntity extends IndexedEntity<Entry> {
  ...
  static async listByJournal(env: Env, journalId: string, requester: string) {
    const { items } = await this.list(env);
    return items.filter(e => e.journalId === journalId && (requester === "public" || e.userId === requester));
  }
  static async deleteManyByUser(env: Env, userId: string) {
    const items = await this.listByUser(env, userId);
    if (items.length) await this.deleteMany(env, items.map(i => i.id));
...
export class ExportLogEntity extends IndexedEntity<ExportLog> {
  ...
  static readonly initialState: ExportLog = { id: "", userId: "", journalId: "", timestamp: "", format: "pdf", status: "success" };
  static async listByUser(env: Env, userId: string) {
    const { items } = await this.list(env);
    return items.filter(l => l.userId === userId).sort((a, b) => b.timestamp.localeCompare(a.timestamp));
  }
  static async deleteManyByUser(env: Env, userId: string) {
    const items = await this.listByUser(env, userId);
    if (items.length) await this.deleteMany(env, items.map(i => i.id));
...
export class LegacyAuditLogEntity extends IndexedEntity<LegacyAuditLog> {
  ...
  static readonly initialState: LegacyAuditLog = { id: "", userId: "", shareId: "", journalId: "", recipientEmail: "", action: "view", timestamp: "" };
  static async listByUser(env: Env, userId: string) {
    const { items } = await this.list(env);
    return items.filter(l => l.userId === userId).sort((a, b) => b.timestamp.localeCompare(a.timestamp));
  }
...
export class SecurityLogEntity extends IndexedEntity<SecurityLog> {
  ...
  static readonly initialState: SecurityLog = { id: "", userId: "", event: "", ip: "", userAgent: "", timestamp: "" };
  static async listByUser(env: Env, userId: string) {
    const { items } = await this.list(env);
    return items.filter(l => l.userId === userId).sort((a, b) => b.timestamp.localeCompare(a.timestamp));
  }
